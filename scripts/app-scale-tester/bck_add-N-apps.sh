...
interval=10
pageSize=10           # Adjust pageSize as needed for querying elements on APIs
offset=0
runTests=1
...
## App install specific variables
containerDeploymentPackageName="dummy-app-package"
containerAppName="dummy-app"
containerAppVersion="0.0.1"
...
deleteDeployments() {
    arr=()
    pageSize=10  # Define the page size (number of deployments per page)
    offset=0     # Start with the first page

    # Fetch total number of deployments
    delTotalAppCnt=$(curl -s "$appOrchEndpoint/v1/projects/$project/appdeployment/deployments" -H 'Content-Type: application/json' -H 'accept: application/json' -H "Authorization: Bearer $orchAPIToken" | jq '.totalElements')

    echo "delTotalAppCnt : $delTotalAppCnt"
    # Calculate the total number of pages needed
    totalPages=$(( (delTotalAppCnt + pageSize - 1) / pageSize ))

    # Loop through each page
    for ((page = 0; page < totalPages; page++)); do
        offset=$((page * pageSize))
        
        # Fetch deployments for the current page
        deployments=$(curl -s "$appOrchEndpoint/v1/projects/$project/appdeployment/deployments?pageSize=${pageSize}&offset=${offset}" -H 'Content-Type: application/json' -H 'accept: application/json' -H "Authorization: Bearer $orchAPIToken")
        deploymentCnt=$(curl -s "$appOrchEndpoint/v1/projects/$project/appdeployment/deployments?pageSize=${pageSize}&offset=${offset}" -H 'Content-Type: application/json' -H 'accept: application/json' -H "Authorization: Bearer $orchAPIToken" | jq '.totalElements')

        if [ $deploymentCnt -lt $pageSize ]; then
            pageSize=$deploymentCnt
        fi
        echo "pageSize : $pageSize"

        # Loop through each deployment in the current page using jq
        echo "$deployments" | jq -c '.deployments[]' | while read -r currDeployment; do
            appName=$(echo "$currDeployment" | jq -r '.appName')
            deployId=$(echo "$currDeployment" | jq -r '.deployId')
            echo "delete appname: $appName"
            echo "delete deployId: $deployId"

            # Check if the appName matches the target package name
            if [ "$appName" == "$containerDeploymentPackageName" ]; then
                arr+=("$deployId")
                echo "deleting deployment: $deployId"
            fi
        done
    done

    # Delete each deployment in the array
    for d in "${arr[@]}"; do
        echo "dep in array: $deployId"
        response=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$appOrchEndpoint/v1/projects/$project/appdeployment/deployments/$d" -H "Authorization: Bearer $orchAPIToken")
        echo "response: $response"
        if [ "$response" -eq 200 ]; then
            echo "Deleted deployment $d..."
        else
            echo "Failed to delete deployment $d. HTTP response code: $response"
        fi
    done
    sleep 2
}
...
waitForAllAppsToBeRunning() {
  ...
  pageSize=10  # Define the page size (number of deployments per page)
  interval=5   # Define the interval between checks (in seconds)
  ...
  while true; do
    ...
    echo "totalAppCnt : $totalAppCnt"
    # Calculate the total number of pages needed
    totalPages=$(( (totalAppCnt + pageSize - 1) / pageSize ))
    ...
    if [ "$totalCnt" -eq "$runningCnt" ]; then
      echo "All $totalCnt apps are running!!"
      break
    fi
