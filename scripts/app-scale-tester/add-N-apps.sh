...
## App install specific variables
containerDeploymentPackageName="nginx-app-local"
containerAppName="nginx-local"
...
catalogDeploy() {
  ...
  case ${NAME} in
    ...
    nginx-app-local)
      catalog ${CATALOG_ARGS} create deployment \
        ${NAME} ${VERSION} --application-label nginx-local.$targetClusterLabel
    ;;
    *)
...
#echo "statusval print : $statusVal"
      ...
      if [[ "$appName" == "$containerDeploymentPackageName" && "$statusVal" == "RUNNING" && "$statusInstance" == "101"  ]]; then
