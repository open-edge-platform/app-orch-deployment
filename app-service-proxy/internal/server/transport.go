...
func rewriteURL(url *url.URL, sourceURL *url.URL, ci *CookieInfo, kubeapiAddr *url.URL) string {
	...
	// Reject paths starting with /\ or // to prevent open redirect via backslash or protocol-relative tricks.
	// Note: protocol-relative URLs (//evil.com) are also caught by isDifferentHost above since
	// net/url parses evil.com into url.Host.
