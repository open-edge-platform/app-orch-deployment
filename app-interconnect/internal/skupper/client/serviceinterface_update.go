...
import (
	...
	"github.com/open-edge-platform/app-orch-deployment/app-interconnect/internal/skupper/pkg/kube/qdr"
	...
	if err == nil {
		...
		if service.IsAnnotated() && kube.IsOriginalServiceModified(service.Address, namespace, cli.GetKubeClient()) {
			...
			for _, t := range service.Targets {
				targetFound := (t.Name == targetName && t.Service == "") && t.Namespace == namespace
				targetServiceFound := t.Service == targetName && t.Namespace == ""
				if targetFound || targetServiceFound {
					modified = true
				} else {
