...
func TestNewClusterClient_EmptyKubeConfig(t *testing.T) {
	// failed scenario with wrong kubeconfig
	_, err := NewClusterClient("")
	assert.Error(t, err)
}

func TestNewClusterClient_FailedCreatingRestConfig(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestNewClusterClient_FailedCreatingNewClientSet(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
