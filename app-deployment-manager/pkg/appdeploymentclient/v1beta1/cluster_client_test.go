...
var _ = Describe("Test Cluster client", func() {
	Describe("Calling API Methods", func() {
		...
		It("successfully list clusters object", func() {
			var opts metav1.ListOptions

			clusters := adClient.Clusters("test_namespace")
			Expect(clusters).NotTo(BeNil())
			resp, err := clusters.List(ctx, opts)
			Expect(resp).NotTo(BeNil())
			Expect(err).ShouldNot(HaveOccurred())
			defer cancel()
		})

		It("successfully get clusters object", func() {
			var opts metav1.GetOptions

			clusters := adClient.Clusters("test_namespace")
			Expect(clusters).NotTo(BeNil())
			resp, err := clusters.Get(ctx, "test", opts)
			Expect(resp).NotTo(BeNil())
			Expect(err).ShouldNot(HaveOccurred())
